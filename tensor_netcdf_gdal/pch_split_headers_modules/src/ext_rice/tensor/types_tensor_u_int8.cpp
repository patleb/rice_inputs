...
namespace Tensor {
      UInt8::View::View(const std::gslice_array< uint8_t > & slice, const Vsize_t & shape):
        size(Base::size_for(shape)),
        slice(slice) {
      }
      void UInt8::View::operator=(const uint8_t & value) const {
        this->slice = value;
      }
      void UInt8::View::operator=(const UInt8 & tensor) const {
        if (size != tensor.size) throw RuntimeError("size[" S(size) "] != tensor.size[" S(tensor.size) "]");
        this->slice = tensor.array;
      }
      auto & UInt8::View::operator=(const View & view) const {
    ...
    UInt8::UInt8(const Vsize_t & shape, const Ouint8_t & fill_value):
      Base::Base(shape),
      fill_value(fill_value.value_or(0)),
      array(this->fill_value, this->size) {
      sync_refs();
    }
    UInt8::UInt8(const Vuint8_t & values, const Vsize_t & shape, const Ouint8_t & fill_value):
      Base::Base(shape),
      fill_value(fill_value.value_or(0)),
    ...
    UInt8::UInt8(const UInt8 & tensor):
      Base::Base(tensor),
      fill_value(tensor.fill_value),
      array(tensor.array) {
      sync_refs();
    }
    UInt8 UInt8::operator+(uint8_t value) const {
      return UInt8(array + value, shape, fill_value);
    }
    UInt8 UInt8::operator+(const UInt8 & tensor) const {
      if (size != tensor.size) throw RuntimeError("size[" S(size) "] != tensor.size[" S(tensor.size) "]");
    ...
    bool UInt8::operator==(const Tensor::Base & tensor) const {
    ...
    UInt8::View UInt8::slice(const Vsize_t & start, const Vsize_t & count, const Vsize_t & stride) {
    ...
    UInt8 UInt8::slice(const Vsize_t & start, const Vsize_t & count, const Vsize_t & stride) const {
      size_t offset = offset_for(start);
      auto counts = counts_or_ones(count);
      auto strides = counts_or_ones(stride);
      auto slice = std::gslice(offset, counts, strides);
    ...
    UInt8 & UInt8::refill_value(uint8_t fill_value) {
    ...
    UInt8 & UInt8::reshape(const Vsize_t & shape) {
      Base::reshape(shape);
      return *this;
    }
    UInt8 & UInt8::seq(const Ouint8_t & start) {
      std::iota(begin(), end(), start.value_or(0));
      return *this;
    }
    size_t UInt8::type_size() const{
      return sizeof(uint8_t);
    }
    UInt8 UInt8::from_sql(const std::string & values, const Vsize_t & shape, const GType & fill_value) {
    ...
    std::string UInt8::to_sql(const Ostring & before, const Ostring & after, Obool nulls) const {
      auto data = this->data();
    ...
    std::string UInt8::to_string(Osize_t limit) const {
    ...
    UInt8 UInt8::atan2(const UInt8 & y, const UInt8 & x) {
