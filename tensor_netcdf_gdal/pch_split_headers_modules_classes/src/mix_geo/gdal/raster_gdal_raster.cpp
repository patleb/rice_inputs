...
namespace GDAL {
      string Raster::Transform::cache_key(const Raster & raster) const {
        return raster.cache_key()
          + std::format(":{}:{}:{}:{}:{}:{}:{}:{}:{}", width, height, x0, y0, dx, dy, rx, ry, reinterpret_cast< std::uintptr_t >(mesh.srs));
      }
    Raster::Raster(Tensor::Base & z, Tensor::Type type, const Vdouble & x01_y01, const Ostring & proj):
    ...
    Vdouble Raster::x() const {
      Vdouble x(width);
      double xi = x0;
      for (size_t i = 0; i < width; ++i, xi += dx) x[i] = xi;
      return x;
    }
    Vdouble Raster::y() const {
      Vdouble y(height);
      double yi = y0;
      for (size_t i = 0; i < height; ++i, yi += dy) y[i] = yi;
      return y;
    }
    Raster Raster::reproject(const string & proj, const GType & fill_value, Obool compact, Obool memoize) const {
      ...
      case Tensor::Type::Int32: {
        auto src_nodata = *reinterpret_cast< const int32_t * >(tensor.nodata);
        auto dst_nodata = is_none(fill_value) ? src_nodata : g_cast< int32_t >(fill_value);
        auto src_data = reinterpret_cast< const int32_t * >(tensor.data());
      ...
      case Tensor::Type::Int64: {
        auto src_nodata = *reinterpret_cast< const int64_t2 * >(tensor.nodata);
        auto dst_nodata = is_none(fill_value) ? src_nodata : g_cast< int64_t2 >(fill_value);
        auto src_data = reinterpret_cast< const int64_t2 * >(tensor.data());
        auto dst_z = Tensor::build(type(), { height, width }, g_cast(dst_nodata));
        auto & dst_tensor = Tensor::cast(dst_z);
        auto dst_data = reinterpret_cast< int64_t2 * >(dst_tensor.data());
        bool src_isnan_nodata = std::isnan(src_nodata);
        for (size_t j = 0; j < height; ++j) {
          for (size_t i = 0; i < width; ++i, ++dst_data) {
            auto point = nearest[j][i];
      ...
      case Tensor::Type::SFloat: {
        auto src_nodata = *reinterpret_cast< const float * >(tensor.nodata);
        auto dst_nodata = is_none(fill_value) ? src_nodata : g_cast< float >(fill_value);
        auto src_data = reinterpret_cast< const float * >(tensor.data());
        auto dst_z = Tensor::build(type(), { height, width }, g_cast(dst_nodata));
        auto & dst_tensor = Tensor::cast(dst_z);
      ...
      case Tensor::Type::DFloat: {
        auto src_nodata = *reinterpret_cast< const double * >(tensor.nodata);
        auto dst_nodata = is_none(fill_value) ? src_nodata : g_cast< double >(fill_value);
        auto src_data = reinterpret_cast< const double * >(tensor.data());
        auto dst_z = Tensor::build(type(), { height, width }, g_cast(dst_nodata));
        auto & dst_tensor = Tensor::cast(dst_z);
        auto dst_data = reinterpret_cast< double * >(dst_tensor.data());
        bool src_isnan_nodata = std::isnan(src_nodata);
        for (size_t j = 0; j < height; ++j) {
          for (size_t i = 0; i < width; ++i, ++dst_data) {
            auto point = nearest[j][i];
      ...
      case Tensor::Type::UInt8: {
        auto src_nodata = *reinterpret_cast< const uint8_t * >(tensor.nodata);
        auto dst_nodata = is_none(fill_value) ? src_nodata : g_cast< uint8_t >(fill_value);
        auto src_data = reinterpret_cast< const uint8_t * >(tensor.data());
        auto dst_z = Tensor::build(type(), { height, width }, g_cast(dst_nodata));
        auto & dst_tensor = Tensor::cast(dst_z);
        auto dst_data = reinterpret_cast< uint8_t * >(dst_tensor.data());
        bool src_isnan_nodata = std::isnan(src_nodata);
        for (size_t j = 0; j < height; ++j) {
          for (size_t i = 0; i < width; ++i, ++dst_data) {
            auto point = nearest[j][i];
      ...
      case Tensor::Type::UInt32: {
        auto src_nodata = *reinterpret_cast< const uint32_t * >(tensor.nodata);
        auto dst_nodata = is_none(fill_value) ? src_nodata : g_cast< uint32_t >(fill_value);
        auto src_data = reinterpret_cast< const uint32_t * >(tensor.data());
        auto dst_z = Tensor::build(type(), { height, width }, g_cast(dst_nodata));
        auto & dst_tensor = Tensor::cast(dst_z);
        auto dst_data = reinterpret_cast< uint32_t * >(dst_tensor.data());
        bool src_isnan_nodata = std::isnan(src_nodata);
        for (size_t j = 0; j < height; ++j) {
          for (size_t i = 0; i < width; ++i, ++dst_data) {
            auto point = nearest[j][i];
    ...
    Raster::Transform Raster::transform_for(const string & proj, Obool compact, Obool memoize) const {
    ...
    string Raster::cache_key() const {
      return std::format("{}:{}:{}:{}:{}:{}:{}", width, height, x0, y0, dx, dy, reinterpret_cast< std::uintptr_t >(srs));
    }
    auto Raster::cache_key_for(const string & proj, bool compact) const {
      return cache_key() + std::format(":{}:{}", reinterpret_cast< std::uintptr_t >(srs_for(proj)), compact);
    }
    Raster::Transform Raster::cached_transform_for(const string & proj, bool compact) const {
    ...
    vector< Vssize_t > Raster::nearest_for(const Transform & tf, Obool memoize) const {
    ...
    vector< Vssize_t > & Raster::cached_nearest_for(const Transform & tf) const {
